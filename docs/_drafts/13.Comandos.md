...
* El nombre de las figuras se guarda en `\figurename`[^name].

* Usamos un *espacio duro* entre el nombre y el número para que LaTeX
  no meta en medio un salto de línea o de página.

[^name]: Del mismo modo, el nombre de los capítulos se guarda en
`\chaptername` y el de las tablas en `\tablename`. Cuidado porque no
todos los elementos siguen este patrón, de hecho, `\sectionname` no
existe.
...
Vamos ahora con los entornos, al igual que con los comandos
empezaremos creando entornos nuevos y luego modificaremos los que ya
hay.

### Entornos nuevos

La sintaxis para los entornos nuevos es similar a la de los comandos:
...
(`\hrule`) y nos permite darle un título que aparecerá en negrita:
...
Se usa en el cuerpo del documento como cualquier otro entorno:
...
De esta manera, si escribimos algo de este estilo:
...
para los entornos tenemos `\renewcommand`:
...
como dos comandos, uno que da inicio al formato concreto y uno que lo
...
En definitiva, creo que la única manera de aprender a modificar
entornos es modificar entornos.
...
hay que hacer lo uno o lo otro. Tampoco está de más recordar que tanto
las definiciones como las modificaciones las hacemos en el
...
* Traslado la combinación al preámbulo y le doy forma según la
  sintaxis correspondiente.

* Extraigo los argumentos y pienso si puedo darle un valor por defecto
  a alguno de ellos.
